...
module prepares the bounded deep-dive windows, builds one ``scout_a09_model_task@1``,
...
from g02.scout_a07_runner import command_executor, parse_model_json  # noqa: E402
...
def build_a09_task(
    ...
    *,
    ...
    max_deep_dive_sources: int = scout_synthesis.DEFAULT_MAX_DEEP_DIVE_SOURCES,
    deep_dive_windows: int = scout_synthesis.A09_DEEP_DIVE_WINDOWS,
    deep_dive_chars: int = scout_synthesis.A09_DEEP_DIVE_CHARS,
) -> dict:
    """Prepare synthesis input, bounded deep-dive windows and one A09 model task."""
    if deep_dive_windows < 1 or deep_dive_windows > scout_synthesis.A09_DEEP_DIVE_WINDOWS:
        raise ValueError("A09 deep_dive_windows must be between 1 and 8")
    if deep_dive_chars < 1 or deep_dive_chars > scout_synthesis.A09_DEEP_DIVE_CHARS:
    ...
    deep_dive = scout_synthesis.gather_deep_dive_windows(
        synthesis_input["reviews"],
        synthesis_input["deep_dive_requests"],
        max_windows=deep_dive_windows,
        max_chars=deep_dive_chars,
    )
    task = scout_synthesis.build_scout_a09_model_task(synthesis_input, deep_dive)
    ...
    *,
    ...
    if executor is not None:
        try:
            ...
            model_output = scout_synthesis.validate_scout_a09_output(model_output)
...
def main(argv: list[str] | None = None) -> int:
    ...
    prepare = sub.add_parser("prepare-task", help="Write the scout_a09_model_task@1 JSON")
    prepare.add_argument("reviews_json")
    prepare.add_argument("--intake", default="")
    prepare.add_argument("--max-deep-dive-sources", type=int,
                         default=scout_synthesis.DEFAULT_MAX_DEEP_DIVE_SOURCES)
