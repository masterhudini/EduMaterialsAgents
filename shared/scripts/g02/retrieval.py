...
ACCEPTANCE_CRITERIA = [
    ...
    for index, description in enumerate([
        ...
        "Market cases use only gated A11 extraction and remain marked as untrusted external content.",
    ], 1)
]
EVIDENCE_REQUIREMENTS = [
    ...
    {"requirement_id": "RT-E03", "mandatory": True,
     "description": "Every market-case file traces to one approved A11 extraction result and content hash."},
]
...
def _copy_market_case(retrieval_input: dict, approved: dict, extract: dict,
    ...
    run_dir = (config.retrieval_accepted_dir / _safe(retrieval_input["task_id"])
               ...
               / "market-cases")
    path = run_dir / f"{_safe(source_id)}.market-case.json"
    _atomic_json(path, payload)
    digest = hashlib.sha256(path.read_bytes()).hexdigest()
    return {
        "source_id": source_id, "status": "accepted", "file_type": "market_case_json",
        "local_ref": oa_retrieval.corpus_ref(path, config), "sha256": digest,
...
def finalize_retrieval(retrieval_input: dict, result_refs: list[str], *,
    ...
    try:
        ...
        corpus = {
            ...
            "retrieval_summary": {
                ...
                "market_case_file_count": len(market_cases),
    ...
    return _envelope(
        status,
        f"Stored {len(documents)} scholarly document(s) and {len(market_cases)} market-case file(s).",
        [],
        produced=[
            {"type": "retrieved_corpus", "path": corpus_ref,
             "schema_version": OUTPUT_CONTRACT, "artifact_version": artifact_version},
            {"type": "retrieval_directory", "path": corpus["run_directory_ref"],
             "schema_version": "retrieval_directory@1", "artifact_version": artifact_version},
        ],
...
def validate_retrieved_corpus(corpus: object, retrieval_input: dict, *,
    ...
    for index, case in enumerate(corpus.get("market_cases", [])):
        try:
            path = oa_retrieval.resolve_corpus_ref(case.get("local_ref"), config)
