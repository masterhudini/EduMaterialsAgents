...
2. Loop on the response:
   - **`awaiting_node`**: hydrate `upstream` refs with `intake_get_artifact` if you need them (e.g. the
     ...
     Take `produced[0].path` and call `intake_resume(run_token, node_results={node: ref})`. If you
     genuinely cannot produce the node, call `intake_resume(run_token, node_failures={node: {summary,
     issues}})`.
   - **`awaiting_review`**: review the `artifact_ref` against `review_profile` + the node's acceptance
     ...
     `intake_resume(run_token, review_decisions={node: {decision, findings}})` where decision is
   ...
   - **`awaiting_user`**: present the gate, collect the required decisions, then
     `intake_resume(run_token, decisions={gate: ...})`.
