...
- Used by the `g03-a04-prompt-builder` node when the change-plan gate chose `target_tool=gamma`.
- Call `solution_prompt_build` with the approved `slide_design_set@1` (upstream ref) and
  `target_tool="gamma"` for the draft; refine the wording to Gamma's card-outline idiom.
