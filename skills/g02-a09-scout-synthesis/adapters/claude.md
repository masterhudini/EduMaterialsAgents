...
- Call `research_scout_a09_task_prepare`, then use the `g02-a09-scout-synthesis` skill exactly once
  for the returned `scout_a09_model_task@1`, with model opus / effort medium.
...
- Call MCP `research_scout_synthesis_finalize` with the same `reviews_json`, `intake`, the
