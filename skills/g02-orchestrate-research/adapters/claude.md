...
- Invoke A10 once per producer execution. After `REVISE`, run the producer once more and preserve
  the deterministic revision-completion receipt without another A10 call.
...
- DOI verification → `research_doi_verify` or `research_doi_verify_batch` on unchanged records;
