...
- Used by the `g03-a04-prompt-builder` node when the change-plan gate chose `target_tool=gpt_pro`.
- Call `solution_prompt_build` with the approved `slide_design_set@1` (upstream ref) and
  `target_tool="gpt_pro"` for the draft; refine the wording into a self-contained generation prompt.
