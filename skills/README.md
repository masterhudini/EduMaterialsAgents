...
- `research-orchestrator/SKILL.md` — hosts the Research Graph run: relays each agent's
  `needs_input` to the user, drives reviewer loops, presents the Human Research Gate
  decisions (§9), and freezes the `HumanApprovedResearchBundle` via `core/gate.py`.
