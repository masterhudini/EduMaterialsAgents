...
2. **Research** — get and follow the `research-scout-e2e` MCP prompt with
   ...
   `research_bundle_finalize` returns `RESEARCH_BUNDLE_REF` (the approved
...
Each graph's human gates (user intake gate; G02 Human Research Gate; user solution gate) fire inside
their loops — present them and collect decisions; never auto-approve.
