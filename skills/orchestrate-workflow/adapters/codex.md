...
4. Report the final blueprint ref and each stage's boundary ref. Optional token/timing roll-up per
   stage: `intake_trace` / `research_trace` / `solution_trace` with that stage's `resume_token`.

Each graph's human gates fire inside their loops — present them, collect decisions, never
auto-approve. Do NOT use the nested `*_run_codex` tools inside a Codex session.
